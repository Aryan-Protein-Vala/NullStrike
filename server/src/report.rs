...
pub async fn build_neo4j_graph(app: &crate::app::AppState) -> anyhow::Result<()> {
    // Attempt to connect to local Neo4j
    let graph = match neo4rs::Graph::new("bolt://127.0.0.1:7687", "neo4j", "password").await {
        Ok(g) => g,
        Err(_) => {
            println!("Failed to connect to Neo4j on localhost:7687. Ensure a Neo4j docker container is running.");
            return Ok(());
        }
    };

    for target in &app.targets {
        graph.run(neo4rs::query("MERGE (t:Target {name: $name})").param("name", target.clone())).await?;
    }
    ...
    for res in &app.results {
        if res.is_vulnerable() {
            let q = neo4rs::query("
                ...
                MERGE (v:Vulnerability {name: $check_name})
                MERGE (t)-[:VULNERABLE_TO]->(v)
            ")
            .param("target", res.target().to_string())
            .param("check_name", res.check_name().to_string());
            
            let _ = graph.run(q).await;
        }
    }
    
    println!("Graph successfully exported to Neo4j!");
    Ok(())
}
